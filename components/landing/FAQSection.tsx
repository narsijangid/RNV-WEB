            ...
            <div className="max-w-4xl mx-auto">
                ...
                <Accordion type="single" collapsible className="w-full space-y-4">
                    {faqs.map((faq, index) => (
                        ...
                        >
                            <AccordionTrigger className="flex flex-1 items-center justify-between w-full py-5 text-left font-semibold text-base md:text-lg text-slate-900 transition-all hover:text-orange-500 group">
                                {faq.question}
                                <ChevronDown className="h-5 w-5 shrink-0 text-slate-400 transition-transform duration-300 group-data-[state=open]:rotate-180 group-hover:text-orange-500" />
                            </AccordionTrigger>
                            <AccordionContent className="overflow-hidden text-slate-600 text-sm md:text-base leading-relaxed pb-6 data-[state=closed]:animate-accordion-up data-[state=open]:animate-accordion-down">
                                {faq.answer}
                            </AccordionContent>
                        </AccordionItem>
                    ))}
